...
#         # self.graph_widget.setXRange(5, 20, padding=0)
#         # self.graph_widget.setXRange(self.t-1000, self.t+100, padding=0)
#         self.graph_widget.setBackground(background=None)

#         # graph_g_layout.setXRange(5, 20, padding=0)
...
#         cursor.execute('SELECT pid,start, total_time FROM data WHERE total_time != "0:00" AND USER <>  "root" ORDER BY pid LIMIT 50')  
...
class View(QMainWindow):
    ...
    def extract_db(self):
        ...
        cursor.execute('SELECT pid,start, total_time FROM data WHERE total_time != "0:00" and user != "root" ORDER BY pid LIMIT 10')  
        data = cursor.fetchall()
        return data
